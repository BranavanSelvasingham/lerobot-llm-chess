...
and list the command templates, explicit clone/fetch/checkout/intake/checker
command steps, plus flat external/vendor review requirements for either a local
external checkout or a vendored locked bundle. The intake summary and artifact
index expose digest row counts, selected model digest row count, requirement row
count, selected row count, unselected row count, selected row IDs, command-plan
status, and selected option command count for undecided, external-source, and
vendor-lock decisions so the digest CSV, requirement CSV, and command plan
cannot silently drift from the recorded operator decision.
