...
That still stays hardware-free. The suite forwards the same value to
`scripts/smoke_sim_so101_model_contract.py --model-path` and
`scripts/smoke_sim_ik_reachability_drill.py --model-path`, preserves both child
commands under `child_commands.so101_model_contract.command` and
