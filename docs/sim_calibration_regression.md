...
complete-source-review, and ambiguous-authoritative-root cases, and writes
...
to `source_authority_ready`; every case keeps `source_intake_not_authority`,
...
do not attempt motion, `--require-ready-reviewed-model` fails closed, and ready
...
path, a not-ready placeholder manifest, and a ready synthetic fixture manifest
whose MuJoCo/SimRobot motion check must remain
...
- `so101_training_readiness_gate.status: "serious_training_blocked"` until reviewed model authority, reviewed model-backed board-source pick/place, and policy-ready rollout evidence are all true; the gate must expose `ready`, blocker count/list, `reviewed_model_authority_ready`, `reviewed_model_backed_board_source_pick_place`, `board_pick_reviewed_model_authority_ready`, raw `rollout_ready_for_policy_training`, computed `rollout_policy_training_authority_ready`, `development_fixture_evidence_not_policy_training_truth`, and summary/checklist/README artifact paths
