...
The standalone `scripts/smoke_sim_so101_model_bundle_probe_matrix.py` smoke
...
that are directories or have unsupported suffixes. The selected path must be an
...
plan, the operator command-plan JSON, the recorded external/vendor intake
...
`ready_for_model_backed_ik` fields mirror `ready`, while
`observed_evidence_is_policy_training_authority` and
`ready_for_policy_training` remain false because clearing reviewed model
authority only advances the priority queue to MuJoCo scene validity. The suite
also writes the same
...
SO-101 authority from fixture-only automation coverage. Ready-shaped motion
evidence that still carries handoff `missing_inputs` or pending actions must
report `reviewed_mujoco_bundle_handoff_blocked_open_work` and
`handoff_blocked_open_work` instead of unblocking downstream scene, Gymnasium,
pick/place, or training gates. Focused usage:
...
`place_z_tolerance_m`; the `release_place` row must also carry
lower-before-release contact, board-contact, target-XY, and placement-Z
evidence. Legacy booleans alone cannot unblock serious training.
...
The reviewed-model-authority gate also writes
`so101_reviewed_model_authority_operator_actions.json` and `.csv` beside the
blocker packet. These rows convert each missing authority item into an
operator-facing command template while keeping
`operator_actions_not_authority`, false physical-SO-101 truth, and false
policy-training authority flags. Placeholder command values must be replaced
...
validation before treating it as reviewed physical SO-101 truth. The
source-inventory known-source entry now carries operator command templates to
resolve the upstream pin, fetch and detach-checkout the pinned source, run the
public-candidate intake, scan the candidate source, copy a vendor-locked bundle,
probe the selected model, and rerun source inventory after source-authority
review metadata is supplied; these templates remain operator guidance, not
authority evidence. The public
candidate intake smoke now writes
`so101_public_candidate_source_lock_digests.csv`,
`so101_public_candidate_model_file_observations.csv`,
`so101_public_candidate_operator_intake_plan.json`,
`so101_public_candidate_operator_command_plan.json`, and
...
checklist status/next-action/prior-blocker maps. They also expose
`operator_action_immediate_actions_match_blocker_packet`,
`operator_action_immediate_actions_missing_from_blocker_packet`, and
`blocker_packet_actions_missing_from_operator_actions` so reviewers can
distinguish child MuJoCo motion from aggregate reviewed SO-101 authority and
verify priority semantics without opening the gate JSON by hand.
...
reports `model_authority: "model_bundle_probe_matrix_not_authority"`,
`observed_evidence_is_physical_so101_authority: false`, and
...
`ready_for_policy_training: false`. It covers the default
...
The handoff also keeps policy-training authority explicit and false:
...
frame site presence, target marker presence, the required SO-101 joint set,
finite limited ranges for the six controlled joints, both gripper finger
collision geoms, model load, SimRobot sync, and scripted environment
completion. It also covers invalid-square,
...
or actions, explicit action IDs that drift from `next_required_for_goal`,
physical-truth claims, policy-training authority claims, unlimited required joints, or a
`gates_unblocked_when_physical_handoff_ready` list that does not explicitly
include `mujoco_scene_validity`. It also rejects a ready-shaped handoff whose
`next_downstream_gate_after_ready` is not `mujoco_scene_validity`, preserving
the reviewed-MuJoCo -> scene-validity -> Gymnasium -> pick/place order. The
valid ready-handoff case records `reviewed_mujoco_handoff_contract_ok: true`,
`reviewed_mujoco_handoff_priority_contract_ok: true`, and
`reviewed_mujoco_handoff_physical_motion_checked: true`,
`reviewed_mujoco_handoff_joint_limit_enablement_status:
"so101_mujoco_joints_limited"`, empty
`reviewed_mujoco_handoff_missing_limited_joints`,
`reviewed_mujoco_handoff_action_ids_match_next_required: true`, and empty
two-way handoff action-drift lists but still keeps
...
lower-before-release gripper contact, lower-before-release board contact, lower
target XY/Z tolerance, placement without manual piece-pose edits,
release-contact clearance, final board contact, and final target XY tolerance.
It also reports
...
`manual_piece_pose_after_reset_stage_ids` so the scripted
reset/lower/close/lift/transfer/lower/release/retreat sequence is ordered and
manual-pose-free after reset.
...
- `smoke_sim_so101_model_source_inventory.py` records deterministic repo-local SO-101 model-source inventory evidence, candidate/provenance/authority counts, source-authority review scope readiness, source-authority gate status/blockers, review-packet JSON/CSV intake, source-intake checklist JSON/CSV command templates, explicit top-level non-authority/readiness flags, and explicit missing-authoritative-model diagnostics before contract or IK checks.
- `smoke_sim_so101_model_bundle_probe.py` records deterministic SO-101 bundle draft evidence, including the default no-model candidate manifest template, selected candidate path when available, review-packet JSON/CSV intake, child contract/manifest diagnostics, observed source/joint/mesh hints, and explicit top-level non-authoritative draft status.
- `smoke_sim_so101_model_bundle_manifest.py` records deterministic SO-101 model bundle manifest evidence, including default no-manifest diagnostics, reviewed manifest fields, child contract/preflight diagnostics, readiness, top-level physical/policy authority boundary flags, and downstream forwarding decisions. The suite aggregates this with source-inventory and reviewed-MuJoCo evidence under `so101_reviewed_model_authority_gate`, including a blocker-packet JSON/CSV review artifact.
...
- `so101_reviewed_mujoco_bundle.status: "reviewed_mujoco_bundle_not_ready"` in default CI, `"reviewed_mujoco_bundle_motion_checked"` when a ready manifest is supplied, or `"reviewed_mujoco_bundle_handoff_blocked_open_work"` when ready-shaped motion evidence still carries handoff open work, with `reviewed_model_motion_checked`, `observed_evidence_is_physical_so101_authority` mirroring physical-reviewed motion, `development_fixture_evidence_not_physical_so101_truth` mirroring the inverse, `ready_for_policy_training: false`, `observed_evidence_is_policy_training_authority: false`, per-joint motion-check CSV rows, `motion_authority_status`, physical-reviewed motion, fixture-motion, non-physical motion-evidence fields, downstream handoff model-identity checkpoint (`downstream_handoff_model_identity_contract_ok`, status, path, declared SHA-256, observed SHA-256), downstream handoff `missing_inputs`, downstream handoff pending actions, ready-handoff open-work blockers, downstream handoff `policy_training_authority_claimed: false`, downstream handoff `development_fixture_evidence_not_policy_training_truth: true`, and summary/checklist/motion-checks/downstream-handoff/README artifact paths populated. Any raw-ready or fixture-ready downstream handoff without a present/matching model-identity checkpoint, or with non-empty missing inputs or pending actions, must fail closed before scene, Gymnasium, pick/place, or training readiness gates treat it as usable.
- `so101_model_source_inventory.status: "missing_authoritative_model"`, `"ambiguous_authoritative_model"`, or `"authoritative_model_found"` with `model_authority: "model_source_inventory_not_authority"`, candidate counts, authoritative candidate count, authoritative source-selection status, selected authoritative path and SHA-256 digest when exactly one candidate is selected, source-authority review scope readiness/missing scope IDs, source-authority review evidence required/satisfied/missing groups, source-authority gate status/blockers, review-packet status/model-authority/item count/action IDs, known-public-source operator command templates for pin/fetch/checkout/candidate-intake/scan/vendor-lock/probe/reviewed-inventory handoff, source-intake recommended candidate path/root context, source-review requirements status/model-authority/count/action-required IDs, false observed-evidence-as-authority, false policy authority, true development-fixture caveats, false physical-authority-ready/model-backed-IK/policy-training readiness flags, recommended contract-check path when present, and summary/CSV/review-packet/source-intake/source-review-requirements/README artifact paths populated. Accepted source-authority review evidence must provide reviewer identity plus a stable artifact handle (`review_id` or HTTP(S) `review_url`); a lone non-placeholder reviewer string, malformed review URL, future-dated review timestamp, or date-only trace is insufficient for source authority.
- `so101_model_bundle_probe.status: "candidate_model_missing"`, `"candidate_model_unavailable"`, `"candidate_manifest_needs_review"`, or `"candidate_manifest_ready_for_model_backed_ik"` with `model_authority: "draft_candidate_not_reviewed"`, false physical/policy authority, true development-fixture caveats, false physical-authority-ready and policy-training readiness, selected model path, model request status, observed source/joint/mesh hints, manifest status, review-packet status/item count, `missing_inputs`, `next_required_action_ids`, and summary/candidate-manifest/review-packet/checklist/README plus child contract/manifest-check artifacts populated; default CI must keep `ready_for_model_backed_ik: false`
- `so101_reviewed_model_authority_gate.status: "reviewed_model_authority_blocked"` until source authority, physical bundle authority, source-to-bundle model path/digest consistency, and physical-reviewed MuJoCo motion are all true; the gate must expose `model_authority` as `reviewed_model_authority_gate_not_authority` while blocked or `reviewed_so101_model_authority_gate` when ready, `observed_evidence_is_physical_so101_authority` mirroring `ready`, `observed_evidence_is_policy_training_authority: false`, `ready_for_model_backed_ik` mirroring `ready`, `ready_for_policy_training: false`, `ready`, `blockers`, `blocker_count`, ordered `next_required_for_goal`/`next_required_action_ids`, source/bundle/consistency/motion readiness booleans, blocker-packet status/model-authority/item count/action-required IDs/blocked-by-prior IDs, operator-action status/model-authority/counts/immediate action IDs/command scopes with `operator_actions_not_authority`, `operator_action_immediate_actions_match_blocker_packet: true`, empty operator-action/blocker-packet two-way missing-action lists, checklist status/next-action/prior-blocker/prior-status mappings that agree with the blocker packet, `training_priority_gate_id`, `training_priority_gate_order`, `next_training_gate_after_ready`, `blocks_serious_policy_training_until_ready`, `ready_does_not_imply_policy_training_ready`, `policy_training_authority_boundary`, development-fixture physical/policy caveats, and summary/checklist/blocker-packet/operator-action/README artifact paths
...
- `so101_mujoco_scene.status: "ok"` with `model_authority: "development_scaffold_not_reviewed"`, false physical-SO-101 and policy-training authority flags, true development-fixture-not-truth flags, `ready_for_model_backed_ik: false`, `ready_for_policy_training: false`, `mujoco_scene_validity_status: "development_scene_validated_not_physical_authority"`, source/target squares and max-step budget recorded, `square_geom_count: 64`, target-frame site and target marker present, required model joints populated with the six controlled SO-101 joints plus `piece_source_freejoint`, limited-joint evidence populated with the six controlled joints, no missing/unlimited/invalid-range required joints, both gripper collision geoms present, MuJoCo load/sync checks passing, reviewed MuJoCo handoff intake fields populated from the reviewed-bundle downstream handoff, `reviewed_mujoco_handoff_contract_ok`, `reviewed_mujoco_handoff_priority_contract_ok`, `reviewed_mujoco_handoff_priority_gate_id`, `reviewed_mujoco_handoff_priority_gate_order`, `reviewed_mujoco_handoff_next_downstream_gate_after_ready`, `reviewed_mujoco_handoff_joint_limit_enablement_status`, `reviewed_mujoco_handoff_missing_limited_joints`, `reviewed_mujoco_handoff_action_ids_match_next_required`, and empty two-way action-drift lists recorded, `scene_uses_reviewed_mujoco_handoff: false`, and summary/model/manifest/CSV/README artifact paths populated
- `so101_mujoco_scene_matrix` includes invalid-square, same-source/target, non-positive max-step, required not-ready handoff, required fixture-only handoff, forged-ready handoff, stale-schema ready handoff, incomplete-ready handoff, ready-handoff-with-missing-input, ready-handoff-with-pending-action, ready-handoff-action-ID-drift, ready-handoff-with-physical-truth-claim, ready-handoff-with-policy-training-authority-claim, ready-handoff-missing-scene-gate, ready-handoff-wrong-next-downstream-gate, and ready-handoff-with-unlimited-joint-evidence cases that must fail closed, plus optional and required ready-handoff intake cases that must pass while keeping the generated scene development-only. Passing cases assert the required joint set, limited-joint evidence, reviewed-handoff priority contract, reviewed-handoff joint-limit enablement evidence, synchronized handoff action IDs, and gripper collision geoms; fail-closed cases keep authority/readiness flags false, write summary/CSV/README artifacts, and leave model XML and manifest paths absent.
- `so101_chess_env.status: "ok"` with `model_authority: "development_scaffold_not_reviewed"` when the generated MuJoCo model is loaded, false physical-SO-101 and policy-training authority flags, true development-fixture-not-truth flags, `ready_for_model_backed_ik: false`, `ready_for_policy_training: false`, strict Gymnasium/MuJoCo dependencies available, `mujoco_backend_required: true`, `mujoco_backend_loaded: true`, `joint_state_fallback_active: false`, `gymnasium_task_wiring_status: "development_mujoco_env_scripted"`, a valid `gymnasium_api_contract` proving six `float32` controlled-joint actions plus expected reset/final observation keys and shapes, development-scene reviewed-handoff joint-limit fields preserved as not requested/not ready, `training_authority_status: "development_mujoco_env_verified_not_policy_ready"`, serious-training blockers including `reviewed_so101_model_bundle`, symbolic contact model recorded, scripted pick/place complete, and summary/CSV/README artifact paths populated; invalid task configuration or incomplete scripted pick/place must return nonzero with false policy/model readiness
- `so101_env_resets.status: "ok"` with `model_authority: "development_scaffold_not_reviewed"`, false physical-SO-101 and policy-training authority flags, true development-fixture-not-truth flags, `ready_for_model_backed_ik: false`, `ready_for_policy_training: false`, `all_resets_ok: true`,
  `all_mujoco_fallback_free: true`, reset count populated, invalid reset
  requests rejected for invalid source squares, invalid target squares,
  identical source/target squares, malformed task options, and empty sampled
  task pools, recovery after invalid resets passing, and summary/CSV/
  invalid-reset-CSV/model/manifest/README artifact paths populated
...
- `so101_model_source_inventory/so101_model_source_review_requirements.json`
- `so101_model_source_inventory/so101_model_source_review_requirements.csv`
...
- `so101_reviewed_model_authority_gate/so101_reviewed_model_authority_operator_actions.json`
- `so101_reviewed_model_authority_gate/so101_reviewed_model_authority_operator_actions.csv`
