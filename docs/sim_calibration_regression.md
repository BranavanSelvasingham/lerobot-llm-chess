...
The bundle manifest checker also preserves `so101_model_bundle_manifest_intake_checklist.json` and `so101_model_bundle_manifest_intake_checklist.csv`. These files turn the current missing reviewed-bundle fields into rerun command templates and must stay labeled `bundle_manifest_intake_not_authority` with false observed-evidence and physical-truth flags.
