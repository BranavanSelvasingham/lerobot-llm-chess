...
when the manifest reports `ready_for_model_backed_ik: true`. Focused usage:
...
deterministic scripted rollout JSONL/CSV evidence. They are priority automation
...
fixture, requires explicit `--ik-model-path` to take precedence and leave the
ready manifest diagnostic-only for downstream IK/contract forwarding, requires
an incomplete placeholder-alignment manifest to remain not ready and not
forward, and requires `artifact_index.missing_artifact_count: 0` for every suite
case.
...
joint-motion checks before scripted curriculum evidence is trusted. TCP/gripper
...
- `smoke_sim_so101_training_rollouts.py` collects deterministic scripted JSONL/CSV rollout evidence for focused pick/place curriculum debugging.
...
- `so101_chess_env.status: "ok"` with strict Gymnasium/MuJoCo dependencies available, MuJoCo backend loaded, scripted pick/place complete, and summary/CSV/README artifact paths populated
...
- `so101_training_rollouts.status: "ok"` with scripted episodes/transitions populated, `all_mujoco_fallback_free: true`, `all_mujoco_piece_release_synced: true`, and summary/JSONL/CSV/model/manifest/README artifact paths populated
