...
Current gripper-reference SimCamera tuning uses the diagnostics as hardware-free evidence only. The 2026-06-16 owner check under `/private/tmp/lerobot_sim/simcamera_tuning_owner_check` first recorded baseline suite diagnostics with selected comparisons `3`, visual comparisons `1`, metadata-only rows `2`, `media_assets_copied_into_repo: false`, missing depth reference `true`, missing pick/place video `true`, and artifact-index missing artifacts `0`. A deterministic profile sweep with fixed marker time then compared the previous `72px` gripper finger width against the reviewed `fingers_narrow_8px` candidate and promoted the conservative `64px` finger width: the current-profile mean absolute delta against `archive/chess_test_images/current_view.jpg` moved from `74.013` to `73.690`, with board corners still sourced from `camera_metadata_pinhole_projection` and tracked gripper opening still `47px`. The tuned sweep still ranks narrower opening/finger candidates as review prompts, and the profile-corner board-framing candidate remains explicitly ranked rather than physical calibration truth.
