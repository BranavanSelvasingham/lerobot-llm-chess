...
`candidate_operator_command_plan_not_authority` and list the command templates,
explicit clone/fetch/checkout/intake/checker command steps, plus flat
external/vendor review requirements for either a local external checkout or a
vendored locked bundle. The intake summary and artifact index expose the
requirement row count, selected row count, unselected row count, selected row
IDs, command-plan status, and selected option command count for undecided,
external-source, and vendor-lock decisions so the CSV and command plan cannot
