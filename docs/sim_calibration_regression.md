...
The suite also runs `smoke_sim_so101_public_candidate_intake_matrix.py` under `so101_public_candidate_intake_matrix/` after the SO-101 model-source inventory. It preserves `so101_public_candidate_intake_matrix_summary.json`, `so101_public_candidate_intake_matrix_cases.csv`, `README.md`, and nested case summaries, source-lock JSONs, direct review-manifest templates, and seeded manifest-check previews. This matrix documents the public TheRobotStudio/SO-ARM100 SO101 asset intake path as a candidate upstream source only; it remains `public_candidate_intake_matrix_not_authority`, keeps physical-authority and policy-training readiness false, and requires a pinned upstream commit, digest review, single-model selection, joint/TCP/base-board review, and passing reviewed bundle manifest before downstream SO-101 model authority can be claimed.
