...
- `so101_mujoco_scene_matrix` includes invalid-square, same-source/target, non-positive max-step, required not-ready handoff, required fixture-only handoff, forged-ready handoff, and incomplete-ready handoff cases that must fail closed, keep authority/readiness flags false, write summary/CSV/README artifacts, and leave model XML and manifest paths absent for fail-closed cases
