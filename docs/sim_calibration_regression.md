...
- `so101_reviewed_model_authority_gate.status: "reviewed_model_authority_blocked"` until source authority, physical bundle authority, and physical-reviewed MuJoCo motion are all true; the gate must expose `ready`, `blockers`, `blocker_count`, source/bundle/motion readiness booleans, blocker-packet status/model-authority/item count/action IDs, `development_fixture_evidence_not_physical_so101_truth`, and summary/checklist/blocker-packet/README artifact paths
