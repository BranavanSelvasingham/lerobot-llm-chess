...
After the inventory, comparison, reference camera tuning diagnostics, and SimCamera tuning before/after evidence identify the missing real depth and pick/place-video inputs, use [docs/sim_reference_capture_manifest.md](sim_reference_capture_manifest.md) to check an operator-maintained local manifest for those captures. That manifest checker resolves media and sidecar paths without copying assets and reports readiness for future calibration-grade review separately from the current hardware-free image evidence.
