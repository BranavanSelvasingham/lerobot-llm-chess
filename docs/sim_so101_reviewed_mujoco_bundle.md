...
malformed or unavailable `asset_roots` metadata and a ready-looking manifest
whose model still references a mesh that cannot be resolved from the model
directory or declared asset roots; those cases must report `asset_roots` or
