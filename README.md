...
Before simulator, camera profile, or perception calibration changes, run the hardware-free local regression gate documented in [docs/sim_calibration_regression.md](docs/sim_calibration_regression.md). A focused GitHub Actions workflow runs the same gate for relevant pull requests into `feat/telemetry-recording`. The suite records its summary and artifacts under `/private/tmp/lerobot_sim/calibration_regression_suite` locally, includes deterministic SimCamera pose-fixture frames for nominal and perturbed board/camera views, includes a gripper-camera POV visibility/occlusion review bundle, writes visual-review contact sheets under `visual_review/`, includes a distance-annotated simulator pick/place sequence plus metric depth/distance JSON/CSV and a metadata-derived perceived-depth comparison JSON/CSV for pickup calibration review, includes a reference capture checklist for the current real-media gap, includes app-entrypoint SimCamera metadata contract evidence, and skips robot hardware, GUI display, and OpenAI paths.
