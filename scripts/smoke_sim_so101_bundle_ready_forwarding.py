...
def write_readme(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        ...
        f"- `invalid_asset_root_entries_manifest`: `{summary['fixtures']['invalid_asset_root_entries_manifest_path']}`",
        ...
        f"- `invalid_model_sha_manifest`: `{summary['fixtures']['invalid_model_sha_manifest_path']}`",
...
def invalid_asset_root_entries_manifest_payload() -> dict[str, Any]:
    payload = manifest_payload(ready=True)
    payload["asset_roots"] = [42, ""]
    return payload
...
def invalid_target_frame_manifest_payload(model_filename: str) -> dict[str, Any]:
    payload = manifest_payload(ready=True, model_filename=model_filename)
    payload["target_frame"] = ["gripper_frame_link"]
    return payload
...
def create_fixtures(output_dir: Path) -> dict[str, Path]:
    ...
    invalid_target_frame_model_path = invalid_target_frame_dir / "model" / "synthetic_so101_mujoco.xml"
    invalid_target_frame_model_path.write_text(mjcf_with_mesh_reference())
    ...
    invalid_asset_root_entries_manifest_path = (
        invalid_asset_root_entries_dir
        / "so101_model_bundle.invalid_asset_root_entries.json"
    )
    ...
    invalid_target_frame_manifest_path = (
        invalid_target_frame_dir / "so101_model_bundle.invalid_target_frame.json"
    )
    ...
    write_manifest_json(
        invalid_asset_root_entries_manifest_path,
        invalid_asset_root_entries_manifest_payload(),
        invalid_asset_root_entries_dir / "model" / "synthetic_so101.urdf",
    )
    ...
    write_json(
        invalid_model_sha_manifest_path,
        invalid_model_sha_manifest_payload(model_filename="synthetic_so101.urdf"),
    )
    write_json(
        mismatched_model_sha_manifest_path,
    ...
    write_manifest_json(
        invalid_target_frame_manifest_path,
        invalid_target_frame_manifest_payload(model_filename=invalid_target_frame_model_path.name),
        invalid_target_frame_model_path,
    )
    write_manifest_json(
        model_missing_target_frame_manifest_path,
